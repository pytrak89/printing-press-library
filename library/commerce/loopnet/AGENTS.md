# LoopNet Printed CLI Agent Guide

This directory is a generated `loopnet-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
loopnet-pp-cli doctor --json
loopnet-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
loopnet-pp-cli which "<capability>" --json
loopnet-pp-cli <command> --help
```
...
```bash
loopnet-pp-cli <command> --help
loopnet-pp-cli <command> --dry-run --agent
```
