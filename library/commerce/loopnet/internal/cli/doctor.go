...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  loopnet-pp-cli doctor
  loopnet-pp-cli doctor --json
  loopnet-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Check auth. LoopNet's data pages sit behind Akamai Bot
			// Manager; live fetches replay browser-minted clearance
			// cookies. No API key — but live `sync`/`inventory` still
			// need cookies. Report the cookie state so an agent's
			// pre-flight sees the real prerequisite, not "not required".
			if cookie, savedAt := lnLoadCookies(); cookie == "" {
				report["auth"] = "clearance cookies missing — run 'loopnet-pp-cli auth refresh' for live fetches (offline commands work without)"
			} else if !savedAt.IsZero() && time.Since(savedAt) > 6*time.Hour {
				report["auth"] = fmt.Sprintf("clearance cookies stale (%s old) — run 'loopnet-pp-cli auth refresh' if live fetches fail", time.Since(savedAt).Round(time.Hour))
			} else {
				report["auth"] = "clearance cookies present"
			}
			...
			for _, ck := range checkKeys {
				...
				case strings.HasPrefix(s, "clearance cookies missing") || strings.HasPrefix(s, "clearance cookies stale"):
					// Live-fetch prerequisite, not a broken CLI: offline
					// commands work without cookies. WARN, not FAIL.
					indicator = yellow("WARN")
				case strings.HasPrefix(s, "clearance cookies present"):
					indicator = green("OK")
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("loopnet-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'loopnet-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'loopnet-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'loopnet-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'loopnet-pp-cli sync' to refresh."
	}
	return report
}
