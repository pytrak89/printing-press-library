...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".loopnet-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LOOPNET_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LOOPNET_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "loopnet-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.loopnet-pp-cli/feedback.jsonl.
When ` + "`LOOPNET_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LOOPNET_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  loopnet-pp-cli feedback list
  loopnet-pp-cli feedback list --limit 5
  loopnet-pp-cli feedback list --json`,
