...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "loopnet-pp-cli",
		Short: `Loopnet CLI — LoopNet shows you today — this CLI remembers, building the price-cut, days-on-market, and supply trends LoopNet never e…`,
		Long: `Loopnet CLI — LoopNet shows you today — this CLI remembers, building the price-cut, days-on-market, and supply trends LoopNet never e…
...
Highlights (not in the official API docs):
  ...
  • caprate   Reports the cap-rate, NOI, and price-per-square-foot distribution (count, min, median, quartiles, max) for synced for-sale listings in a submarket, and flags listings whose cap rate falls outside the…
...
Health check: run 'loopnet-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("loopnet-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'loopnet-pp-cli profile list')")
	...
	rootCmd.AddCommand(newPropertyPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Transcendence commands — local-store intelligence (Phase 3, hand-built).
	rootCmd.AddCommand(newPriceCutsCmd(flags))
	rootCmd.AddCommand(newDomCmd(flags))
	rootCmd.AddCommand(newVelocityCmd(flags))
	rootCmd.AddCommand(newDelistedCmd(flags))
	rootCmd.AddCommand(newDistressCmd(flags))
	rootCmd.AddCommand(newCapRateCmd(flags))
	rootCmd.AddCommand(newDigestCmd(flags))
	rootCmd.AddCommand(newFeedCmd(flags))
	// Absorbed offline commands.
	rootCmd.AddCommand(newSearchCmd(flags))
	rootCmd.AddCommand(newSQLCmd(flags))
	rootCmd.AddCommand(newBrokersCmd(flags))
	// Clearance-cookie auth (Akamai bot-protection bypass).
