...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "booking-com-pp-cli", "http")
...
func (c *Client) cacheKey(path string, params map[string]string, headers map[string]string) string {
	...
	for _, k := range headerKeys {
		normalizedName := strings.ToLower(strings.TrimSpace(k))
		normalizedValue := strings.TrimSpace(headers[k])
		if normalizedName == "" || normalizedValue == "" {
			continue
		}
		valueHash := sha256.Sum256([]byte(normalizedValue))
		key += "|header:" + normalizedName + "=" + hex.EncodeToString(valueHash[:8])
	}
...
func (c *Client) doInternal(method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" {
			// Cookie-auth detection: booking.com auth-flow stores its session
			// as `Bearer <name1=val1; name2=val2; ...>` because the framework
			// only has one storage slot. Re-route those to the Cookie header,
			// stripping the Bearer prefix the framework added.
			if strings.HasPrefix(authHeader, "Bearer ") &&
				strings.Contains(authHeader, "=") &&
				strings.Contains(authHeader, ";") {
				req.Header.Set("Cookie", strings.TrimPrefix(authHeader, "Bearer "))
			} else {
