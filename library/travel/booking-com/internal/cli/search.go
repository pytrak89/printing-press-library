...
In local mode: searches locally synced data only.`,
  ...
  booking-com-pp-cli search "error timeout"

  # Force local search only
  booking-com-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  booking-com-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  booking-com-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /searchresults.html
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/searchresults.html", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			// Local FTS search
			if dbPath == "" {
				dbPath = defaultDBPath("booking-com-pp-cli")
			}

			db, err := store.OpenWithContext(cmd.Context(), dbPath)
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'booking-com-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "":
				...
				{
					partial, searchErr := searchHotelResourcesLike(cmd.Context(), db, query, limit)
					if searchErr != nil {
						return fmt.Errorf("search hotel resources failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
			...
			default:
				// Unrecognized type — fall back to generic search. For hotels,
				// also query the generic resources table with LIKE because
				// generated sync may store only the generic JSON row.
				results, err = db.Search(query, limit)
				if resourceType == "hotels" && err == nil {
					partial, searchErr := searchHotelResourcesLike(cmd.Context(), db, query, limit)
					if searchErr != nil {
						return fmt.Errorf("search hotel resources failed: %w", searchErr)
					}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/booking-com-pp-cli/data.db)")
...
func searchHotelResourcesLike(ctx context.Context, db *store.Store, query string, limit int) ([]json.RawMessage, error) {
	if limit <= 0 {
		limit = 50
	}
	like := "%" + strings.ToLower(query) + "%"
	rows, err := db.DB().QueryContext(ctx, `SELECT data FROM resources WHERE resource_type='hotels' AND (lower(COALESCE(json_extract(data,'$.name'),'')) LIKE ? OR lower(COALESCE(json_extract(data,'$.description'),'')) LIKE ? OR lower(data) LIKE ?) ORDER BY updated_at DESC LIMIT ?`, like, like, like, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
