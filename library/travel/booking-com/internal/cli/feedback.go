...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".booking-com-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BOOKING_COM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BOOKING_COM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "booking-com-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.booking-com-pp-cli/feedback.jsonl.
When ` + "`BOOKING_COM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BOOKING_COM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  booking-com-pp-cli feedback list
  booking-com-pp-cli feedback list --limit 5
  booking-com-pp-cli feedback list --json`,
