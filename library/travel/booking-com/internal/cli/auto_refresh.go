...
var readCommandResources = map[string][]string{
	"booking-com-pp-cli cars":            {"cars"},
	"booking-com-pp-cli cars list":       {"cars"},
	"booking-com-pp-cli cars get":        {"cars"},
	"booking-com-pp-cli cars search":     {"cars"},
	"booking-com-pp-cli trips":           {"trips"},
	"booking-com-pp-cli trips list":      {"trips"},
	"booking-com-pp-cli trips get":       {"trips"},
	"booking-com-pp-cli trips search":    {"trips"},
	"booking-com-pp-cli wishlist":        {"wishlist"},
	"booking-com-pp-cli wishlist list":   {"wishlist"},
	"booking-com-pp-cli wishlist get":    {"wishlist"},
	"booking-com-pp-cli wishlist search": {"wishlist"},
}
...
func cachePolicy() cliutil.Policy {
	...
	envOptOut := "BOOKING_COM_NO_AUTO_REFRESH"
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale booking-com-pp-cli cache (refresh failed: %v)\n", err)
...
func runAutoRefresh(ctx context.Context, flags *rootFlags, db *store.Store, resources []string) error {
	...
	for _, resource := range resources {
		...
		result := syncResource(c, db, resource, "", false, 1, true, nil)
		if result.Err != nil {
