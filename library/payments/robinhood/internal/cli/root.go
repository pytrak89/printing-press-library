...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "robinhood-pp-cli",
		Short: "Manage robinhood resources via the robinhood API",
		Long: `Manage robinhood resources via the robinhood API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'robinhood-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("robinhood-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().BoolVar(&flags.liveWrite, "live-write", false, "Allow a write command to attempt live execution after the ROBINHOOD_PP_ALLOW_WRITES=1 env gate")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'robinhood-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.deliverSpec, "deliver", "", "Route output to a sink: stdout (default), file:<path>, webhook:<url>")
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 2, "Max requests per second (0 to disable)")
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		if defaultWriteDryRun(cmd, flags) {
			flags.dryRun = true
		}
		return nil
	}
	rootCmd.AddCommand(newCryptoCmd(flags))
	rootCmd.AddCommand(newBrokerageCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "version",
		Short: "Print the robinhood-pp CLI version",
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("robinhood-pp-cli %s\n", version)
		},
	}
}
