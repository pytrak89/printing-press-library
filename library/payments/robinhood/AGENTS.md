# Robinhood Printed CLI Agent Guide

This directory is a generated `robinhood-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
robinhood-pp-cli doctor --json
robinhood-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
robinhood-pp-cli which "<capability>" --json
robinhood-pp-cli <command> --help
```
...
```bash
robinhood-pp-cli <command> --agent
```
...
```bash
robinhood-pp-cli <command> --help
robinhood-pp-cli <command> --dry-run --agent
```
