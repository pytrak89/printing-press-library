# StackAdapt Printed CLI Agent Guide

This directory is a generated `stackadapt-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
stackadapt-pp-cli doctor --json
stackadapt-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
stackadapt-pp-cli which "<capability>" --json
...
```bash
stackadapt-pp-cli <command> --agent
```
...
```bash
stackadapt-pp-cli <command> --help
stackadapt-pp-cli <command> --dry-run --agent
```
