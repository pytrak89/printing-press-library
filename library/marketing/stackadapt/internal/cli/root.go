...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "stackadapt-pp-cli",
		Short: `StackAdapt CLI — agent-native pacing, delivery-drift, and budget-waste analysis over the StackAdapt GraphQL API`,
		Long: `StackAdapt CLI — the first agent-native StackAdapt CLI: pacing, delivery-drift, and budget-waste analysis the dashboard and warehouse connectors can't answer in one call.
...
Health check: run 'stackadapt-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("stackadapt-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'stackadapt-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelBottleneckCmd(flags))
	rootCmd.AddCommand(newNovelDeliveryDriftCmd(flags))
	rootCmd.AddCommand(newNovelPacingCmd(flags))
	rootCmd.AddCommand(newNovelStaleCampaignsCmd(flags))
	// Hand-authored absorbed read commands (StackAdapt GraphQL).
	rootCmd.AddCommand(newAdvertisersCmd(flags))
	rootCmd.AddCommand(newCampaignsCmd(flags))
	rootCmd.AddCommand(newCampaignGroupsCmd(flags))
	rootCmd.AddCommand(newAdsCmd(flags))
	rootCmd.AddCommand(newSegmentsCmd(flags))
	rootCmd.AddCommand(newAccountCmd(flags))
	// Hand-authored local-store layer (offline search / SQL / --data-source local).
	rootCmd.AddCommand(newSyncCmd(flags))
	rootCmd.AddCommand(newSearchCmd(flags))
	rootCmd.AddCommand(newSQLCmd(flags))
	rootCmd.AddCommand(newReportCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newGraphqlPromotedCmd(flags))
