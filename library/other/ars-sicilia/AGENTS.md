# ARS Sicilia Printed CLI Agent Guide

This directory is a generated `ars-sicilia-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ars-sicilia-pp-cli doctor --json
ars-sicilia-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ars-sicilia-pp-cli which "<capability>" --json
ars-sicilia-pp-cli <command> --help
```
...
```bash
ars-sicilia-pp-cli <command> --agent
```
...
```bash
ars-sicilia-pp-cli <command> --help
ars-sicilia-pp-cli <command> --dry-run --agent
```
