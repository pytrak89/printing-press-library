...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/developer-tools/godaddy/internal/client"
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "godaddy-pp-cli",
		Short: "Manage godaddy resources via the godaddy API",
		Long: `Manage godaddy resources via the godaddy API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'godaddy-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("godaddy-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'godaddy-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAbuseCmd(flags))
	rootCmd.AddCommand(newAftermarketCmd(flags))
	rootCmd.AddCommand(newAgentsCmd(flags))
	rootCmd.AddCommand(newApiCustomersCmd(flags))
	...
	rootCmd.AddCommand(newDomainsCustomersCmd(flags))
	rootCmd.AddCommand(newOrdersCmd(flags))
	rootCmd.AddCommand(newParkingCmd(flags))
	rootCmd.AddCommand(newShoppersCmd(flags))
	...
	rootCmd.AddCommand(newAgreementsPromotedCmd(flags))
	rootCmd.AddCommand(newAuctionsAftermarketPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())
	annotateGoDaddyRisk(rootCmd)
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "version",
		Short: "Print the godaddy-pp CLI version",
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("godaddy-pp-cli %s\n", version)
		},
	}
}
