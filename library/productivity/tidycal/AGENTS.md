# Tidycal Printed CLI Agent Guide

This directory is a generated `tidycal-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
tidycal-pp-cli doctor --json
tidycal-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
tidycal-pp-cli which "<capability>" --json
...
```bash
tidycal-pp-cli <command> --help
tidycal-pp-cli <command> --dry-run --agent
```
